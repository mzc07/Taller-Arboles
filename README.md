...
**BigTree** es una librería de Python que facilita el trabajo con estructuras de datos jerárquicas como árboles y grafos acíclicos dirigidos (DAG). Se caracteriza por ser altamente extensible.

Permite:
- Construir estructuras a partir de listas, diccionarios, DataFrames, etc.
- Realizar recorridos (preorden, postorden y por niveles).
- Buscar y filtrar nodos.
- Exportar a múltiples formatos como JSON, diccionarios y gráficos.

Además, no restringe el número de hijos, lo que permite implementar árboles no binarios de forma natural.
